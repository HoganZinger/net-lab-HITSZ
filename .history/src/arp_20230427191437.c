...
{   
    ...
    if(buf->len>8){
        ...
        uint16_t data = (*hdr);
        if(data != arp_init_pkt.hw_type16)    
            flag=0;
        // upper protocol
        hdr = (uint16_t *)(buf->data+2);
        data = (*hdr);
        if(data != arp_init_pkt.pro_type16)   
            flag = 0;
        // Mac length
        if(buf->data[4] != arp_init_pkt.hw_len)  
            flag = 0;
        //检查IP协议地址长度
        if(buf->data[5] != arp_init_pkt.pro_len)     
            flag = 0;
        // arp type
        hdr = (uint16_t*)(buf->data+6);
        data = (*hdr);
        if(data != swap16(ARP_REQUEST) && data != swap16(ARP_REPLY))   
            flag=0;
        if(flag){
            // src IP
            ...
            if(temp_buf){
                // 存在发送到源主机的buf,此时已经建立了src_ip->src_mac的映射
                // 直接发送数据包
