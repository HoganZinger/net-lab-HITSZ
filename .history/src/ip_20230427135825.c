...
void ip_in(buf_t *buf, uint8_t *src_mac)
{
    ...
    if(buf->len>(4*header->hdr_len)){
        //报头检测
        // 检查版本号
        if(header->version!=IP_VERSION_4)   return;
        // 检查总长度字段
        if(swap16(header->total_len16)>buf->len)    return;
        ...
        // 重新计算校验和,将首部转为16bit为单位的数组,计算20个字节数据的校验和
        uint16_t new_checksum=checksum16((uint16_t*)header,20);
        // 检查校验和
        if(new_checksum!=mychecksum)    return;
        header->hdr_checksum16=mychecksum;

        // 检查目的IP地址是否为本机的IP地址
        int flag=1;
        for(int i=0;i<4;i++){
            if(header->dst_ip[i]!=net_if_ip[i]) flag=0;
        }
        if(flag==0) return;
